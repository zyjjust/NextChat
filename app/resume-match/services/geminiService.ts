...
function getAI(): GoogleGenAI {
    ...
    if (keys.length === 0) {
        throw new Error("NEXT_PUBLIC_GEMINI_API_KEY is not configured in .env");
    }
    ...
    console.log(
        `[AI] Using API key starting with: ${selectedKey.substring(0, 8)}... (Total keys: ${keys.length})`,
    );
...
export async function matchResumeToJDs(
    ...
    【岗位ID: ${jd.id}】
    需求名称: ${jd.title}
    >>> 重点澄清 (HSM与客户确认的最终标准 - 最高优先级) <<<: ${jd.parsedData?.keyClarification || "无"}
